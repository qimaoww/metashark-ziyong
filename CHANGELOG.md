...
- 单集标题的中文语言和简繁校验不再仅限于原名为「第 N 集」的情况，新入库、覆盖刷新和其他标题格式也使用同一规则。
- TMDb 单集标题为空、编号占位、语言不符或重复父级剧名/季名时，尝试同一单集的同地区翻译；不将整部剧名或季名当作单集标题。
- 没有有效标题时保留已有的独立单集名，否则使用「第 N 集」，避免空标题再次回退到文件名中的繁体剧名。剧集组映射继续使用映射后的单集查询翻译，显示编号仍保留媒体库内的季集编号。
- 已保存且符合目标中文语言的独立单集名，即使与文件名相同也会保留；已知父级剧名和季名仍会被排除。
- 已经误写的单集需在升级后执行「覆盖所有元数据」刷新；「搜索缺少的元数据」不会主动覆盖已有的非默认标题。
