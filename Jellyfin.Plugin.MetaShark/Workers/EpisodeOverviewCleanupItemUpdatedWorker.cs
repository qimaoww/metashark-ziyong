...
namespace Jellyfin.Plugin.MetaShark.Workers
{
    ...
    public sealed class EpisodeOverviewCleanupItemUpdatedWorker : IHostedService
    {
        private static readonly Action<ILogger, Exception?> LogWorkerStart =
            LoggerMessage.Define(LogLevel.Information, new EventId(1, nameof(StartAsync)), "Starting episode-overview-cleanup item-updated worker.");
        ...
        private static readonly Action<ILogger, Guid, string, ItemUpdateType, Exception?> LogPostProcessFailed =
            LoggerMessage.Define<Guid, string, ItemUpdateType>(LogLevel.Error, new EventId(3, nameof(OnItemUpdated)), "Episode overview cleanup post-process failed for item {Id} path {ItemPath} trigger=ItemUpdated updateReason={UpdateReason}.");
