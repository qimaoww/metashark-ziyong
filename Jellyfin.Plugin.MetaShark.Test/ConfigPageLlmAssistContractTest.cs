...
namespace Jellyfin.Plugin.MetaShark.Test
{
    ...
    public class ConfigPageLlmAssistContractTest
    {
        ...
        public void ShouldRenderApprovedLlmSafetyCopy()
        {
            ...
            Assert.IsTrue(llmBlock.Contains("默认关闭。仅允许纠正 Movie/Series 的 TMDb ID；IMDb、TVDB、Douban 只作为证据或缺失补全，不做覆盖纠正。LLM 只能提出 TMDb 候选，候选必须经对应 API 强验证；验证失败会保留原有 TMDb。启用本开关、全局 LLM 辅助刮削和完整 LLM 连接配置后，手动识别、手动刷新、搜索缺失元数据和显式覆盖刷新可按 TMDb 纠错触发器评估。自动扫描、隐式刷新、计划任务和其他自动流程不会触发；普通 LLM 辅助刮削、文本补全、外部 ID 缺失补全和剧集组映射不会因为覆盖刷新而运行。", StringComparison.Ordinal), "TMDb 纠错开关必须说明默认关闭、仅限 Movie/Series、其他外部 ID 只作证据或缺失补全、API 强验证保留旧 TMDb、显式覆盖刷新可评估，以及 automatic/implicit/scheduled automatic blocked。 ");
        ...
        public void Readme_ShouldDocumentLlmSafetyBoundaries()
        {
            ...
            Assert.IsTrue(readme.Contains("`允许 LLM 辅助纠正错误 TMDb ID`：默认关闭；只允许纠正 Movie/Series 的 TMDb ID。IMDb、TVDB、Douban 只作为证据或缺失补全，不做覆盖纠正。LLM 只能提出 TMDb 候选，候选必须再经对应 API 强验证；验证失败会保留原有 TMDb。启用本开关、全局 `LLM 辅助刮削` 和完整 LLM 连接配置后，手动识别、手动刷新、搜索缺失元数据和显式覆盖刷新可按 TMDb 纠错触发器评估。自动扫描、隐式刷新、计划任务和其他自动流程不会触发；普通 LLM 辅助刮削、文本补全、外部 ID 缺失补全和剧集组映射不会因为覆盖刷新而运行。", StringComparison.Ordinal), "README 必须说明 TMDb 纠错开关默认关闭、仅限 Movie/Series、其他外部 ID 边界、API 强验证保留旧 TMDb、显式覆盖刷新可评估，以及 automatic/implicit/scheduled automatic blocked。 ");
